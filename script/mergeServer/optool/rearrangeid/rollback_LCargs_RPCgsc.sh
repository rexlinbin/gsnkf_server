...
{
	echo_usage "Usage:"
	echo_usage "	rollback lcserver args and rpcfw gsc with group id"
...
if [ `getLcserverProcssNum $GROUP` -eq 0 ]; then
	echo_info "ROLLBACK $GROUP lcserver args and rpcfw gsc in $MAIN";
	
	#check lcserver conf file exist
	if [ `checkLcserverConfExist $GROUP` -eq 0 ]; then
		ssh $MAIN "cp $MERGEOPBAK/lcserver/$SERVERPRE$GROUP.args  $LCSERVERPATH/conf/$SERVERPRE$GROUP.args"
	else
		echo_notice "$GROUP lcserver args exist!";
	fi

	#check rpcfw conf file exist
	if [ `checkMainGsc $GROUP` -eq 0 ]; then 
		ssh $MAIN "cp -r $MERGEOPBAK/rpcfw/$SERVERPRE$GROUP $RPCFWPATH/conf/gsc/$SERVERPRE$GROUP"
	else
		echo_notice "$GROUP rpcfw conf exist!";
	fi
	
	if [ `checkLcserverConfExist $GROUP` -gt 0 ]; then
