 ...
 * $Id: FeedItem.php 55621 2013-07-16 08:28:28Z MingTian $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/item/scripts/FeedItem.php $
...
function readFeedItem($inputDir)
{
	//数据对应表
	$index = 0;
	$arrConfKey = array (
			...
			ItemDef::ITEM_ATTR_NAME_FEED_EXP			=> ($index+=2)-1		//获得经验值	
	);

	$file = fopen("$inputDir/item_feed.csv", 'r');
	echo "read $inputDir/item_feed.csv\n";
	...
	while ( true )
	{
		...
		if (empty($conf[ItemDef::ITEM_ATTR_NAME_FEED_EXP]))
		{
			trigger_error("feed:$data[0] exp is empty!\n");
		}
