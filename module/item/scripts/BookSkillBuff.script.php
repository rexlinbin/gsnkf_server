 ...
 * $Id: BookSkillBuff.script.php 259838 2016-09-01 02:53:24Z BaoguoMeng $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/item/scripts/BookSkillBuff.script.php $
 * @author $Author: BaoguoMeng $(zhangtiantian@babeltime.com)
 * @date $Date: 2016-09-01 02:53:24 +0000 (Thu, 01 Sep 2016) $
 * @version $Revision: 259838 $
 * @brief 
 *  
 **/
...
$csvFile = 'skill_itembook.csv';
$outFileName = 'BOOKSKILLBUFF';
...
$fieldNames = array (
	PropertyKey::ARR_IMMUNED_BUFF					=>			$index++,//附加免疫buffID组
	PropertyKey::PARRY_SKILL						=>			$index++,//替换闪避技能ID
	PropertyKey::DODGE_SKILL						=>			$index++,//替换反击技能ID
	...
	PropertyKey::ARR_PARRY_SKILL					=>			$index++,//闪避技能附加子技能ID
	PropertyKey::ARR_DODGE_SKILL					=>			$index++,//反击技能附加子技能ID
...
while ( true )
{
	...
	$id = intval($data[0]);
	if(count($data) > (count($fieldNames)+1))
	{
	...
	$conf = array();
	foreach($fieldNames as $key => $v)
	{
		if(in_array($key, $replace))
		{
			$conf['replace'][$key]	=	intval($data[$v]);
		}
		else if(in_array($key, $attach))
		{
			$conf['attach'][$key]	=	array_map('intval',str2Array($data[$v], ','));
		}
		else if(in_array($key, $add))
		{
			$conf['add'][$key]		=	array_map('intval', str2Array($data[$v], ','));
		}
		else
		{
			trigger_error ("error field .".$key);
		}
	}
	$confList[$id] = $conf;
}
