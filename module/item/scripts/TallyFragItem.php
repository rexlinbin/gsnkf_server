 ...
 * $Id: TallyFragItem.php 218718 2015-12-30 09:10:11Z MingTian $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/item/scripts/TallyFragItem.php $
 * @author $Author: MingTian $(tianming@babeltime.com)
 * @date $Date: 2015-12-30 09:10:11 +0000 (Wed, 30 Dec 2015) $
 * @version $Revision: 218718 $
 * @brief 
 *  
 **/
function readTallyFragItem($inputDir)
{
	//数据对应表
	$index = 0;
	$arrConfKey = array (
			...
			ItemDef::ITEM_ATTR_NAME_TALLYFRAG_FORM		=> $index++				//合成物品id
	);

	$file = fopen("$inputDir/item_bingfu_fragment.csv", 'r');
	echo "read $inputDir/item_bingfu_fragment.csv\n";
	...
	while ( true )
	{
		...
		{
			trigger_error("tallyfrag:$data[0] num is empty!\n");
		}
		
		//如果合成所需的物品的碎片数不等于碎片物品的可叠加数,则抛出错误
		if ($conf[ItemDef::ITEM_ATTR_NAME_STACKABLE] != $conf[ItemDef::ITEM_ATTR_NAME_FRAGMENT_NUM])
		{
			trigger_error("tallyfrag:$data[0] fragment number is not equal stackable number\n");
		}
		
		if (empty($conf[ItemDef::ITEM_ATTR_NAME_TALLYFRAG_FORM]))
		{
			trigger_error("tallyfrag:$data[0] form id is empty!\n");
		}
		...
		$conf[ItemDef::ITEM_ATTR_NAME_USE_ACQ] = array(
				ItemDef::ITEM_ATTR_NAME_USE_ACQ_ITEMS => array (
						intval($conf[ItemDef::ITEM_ATTR_NAME_TALLYFRAG_FORM]) => 1
				)
		);
		unset($conf[ItemDef::ITEM_ATTR_NAME_TALLYFRAG_FORM]);
