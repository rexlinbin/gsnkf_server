 ...
 * $Id: EnSignactivity.class.php 259698 2016-08-31 08:07:55Z BaoguoMeng $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/activity/signactivity/EnSignactivity.class.php $
 ...
 *  
 **/
class EnSignactivity
{
	//累计登陆解析函数
	public static function readSignactivityCSV( $arr )
	{
		$index = 0;
		$keyArr = array(
				'id' => $index++,
		...
		// 改版前的配置字段是5个,新版的是6个,此处对老版配置做兼容,为了在版本过渡时测试
		$columns = count($arr[0]);
		if (5 == $columns)
		{
			unset($keyArr['cost']);
		...
		foreach ( $arr as $data )
		{
			...
			$allReward = array();
			Logger::debug('arr in foreach is: %s',$conf['rewardArr'] );
	...
	public static function signForSignActivity()
	{
