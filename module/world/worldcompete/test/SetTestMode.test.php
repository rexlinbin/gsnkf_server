 ...
 * $Id: SetTestMode.test.php 202070 2015-10-14 06:29:42Z MingTian $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/world/worldcompete/test/SetTestMode.test.php $
...
class SetTestMode extends BaseScript
{
	...
	{	
		...
		// 设置跨服机crontab
		$arrCron = array();
		$arrCron[] = sprintf('01 * * * * $SIMPLE_BTSCRIPT $SCRIPT_ROOT/WorldCompeteEntry.php reward do');
		$arrCron[] = sprintf('30 * * * * $SIMPLE_BTSCRIPT $SCRIPT_ROOT/WorldCompeteEntry.php team   do');
	...
	public function setMode($arrAddr, $testMode)
	{
		...
		{
			$cmd = sprintf('ssh %s "/bin/sed -i \'/TEST_MODE/{s/[0-9-]\+/%d/;}\' /home/pirate/rpcfw/conf/WorldCompete.cfg.php"', $aAddr, $testMode);
	...
	public static function setCronToMachine($arrCron, $host, $field, $delOld = TRUE)
	{
		...
		$tmpFile = '/tmp/crontab.worldcompete';
		...
		{
			if ($delOld && preg_match(sprintf('/WorldCompeteEntry.php/'), $cronLine))
			{
				continue;
			}
			if ($foundTag == false)
			...
			if ($cronLine == sprintf('#worldcompete %s', $field))
			{
				$foundTag = true;
			}
		}
		...
		{
			$arrBefore[] = '';
			$arrBefore[] = sprintf('#worldcompete %s',$field);
		}
