 ...
 * $Id: readArenaGoods.script.php 188209 2015-07-31 13:17:56Z JiexinLin $
 * 
 **************************************************************************/

 /**
 * @file $HeadURL: svn://192.168.1.80:3698/C/tags/card/rpcfw/rpcfw_1-0-41-55/module/arena/script/readArenaGoods.script.php $
 * @author $Author: JiexinLin $(tianming@babeltime.com)
 * @date $Date: 2015-07-31 13:17:56 +0000 (Fri, 31 Jul 2015) $
 * @version $Revision: 188209 $
 * @brief 
 *  
 **/
...
$inFileName = 'arena_shop.csv';
$outFileName = 'ARENA_GOODS';
...
$arrConfKey = array (
		'goods' => ++$index,
		MallDef::MALL_EXCHANGE_PRESTIGE => ++$index,
...
while (TRUE)
{
		...
		if ( in_array($key, $arrKeyV2, true) )
		{
			...
			else
			{
				...
				foreach( $arr as $value )
				{
					if(!strpos($value, '|'))
					{
						trigger_error( "arena_goods:$data[0] invalid key:$key, value:$value need v2\n" );
					}
					if ( 'goods' == $key )
					{
						//物品id组那一列的解析
	...
	{
		trigger_error("arena goods:$data[0] both exchangeReq: $req, exchangeAcq: $acq is empty!\n");
	}
